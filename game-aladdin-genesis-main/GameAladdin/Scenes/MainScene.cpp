...
void MainScene::LoadContent()
{
	...
	sourceRect.left = 0;
	sourceRect.right = 2269;
	sourceRect.top = 0;
	sourceRect.bottom = 1138;
	//map nền
	sprite = new Sprite(ResourceManager::GetInstance()->GetTextureSultanDungeon(), true, sourceRect);
	sprite->SetPosition(sprite->GetWidth() / 2.0f, sprite->GetHeight() / 2.0f);
	_backgroundTextures.push_back(sprite);
	//4 cây trụ đá
	sourceRect.top = 1139;
	sourceRect.bottom = 2278;
	sprite = new Sprite(ResourceManager::GetInstance()->GetTextureSultanDungeon(), true, sourceRect);
	sprite->SetPosition(sprite->GetWidth() / 2.0f, sprite->GetHeight() / 2.0f);
	_backgroundTextures.push_back(sprite);
	//trụ gỗ và xích
	sourceRect.top = 2279;
	sourceRect.bottom = 3418;
	sprite = new Sprite(ResourceManager::GetInstance()->GetTextureSultanDungeon(), true, sourceRect);
	sprite->SetPosition(sprite->GetWidth() / 2.0f, sprite->GetHeight() / 2.0f);
	_backgroundTextures.push_back(sprite);
